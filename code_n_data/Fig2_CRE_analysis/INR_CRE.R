...
files=list.files(path=INR_path, pattern="bed.gz")
files=files[1:10]

INR=read.delim(paste0(INR_path,files[1]), header=F, stringsAsFactors = F)
INR=left_join(INR, CREsummit[,c(2,4)], by="V4", copy=F, suffix=c("","_ori"))
INR=INR[which(INR$V11>=48),]
for (i in 2:length(files)){
  INR1=read.delim(paste0(INR_path,files[i]), header=F, stringsAsFactors = F)
  INR1=left_join(INR1, CREsummit[,c(2,4)], by="V4", copy=F, suffix=c("","_ori"))
  INR1=INR1[which(INR1$V11>=48),]
  INR=rbind(INR,INR1)}

length(unique(INR$V4))#69165
...
colnames(INR)[c(4,5,6)]=c("CREID","name","motif_score")
write.table(INR[order(INR$locS),],gzfile(paste0(INR_path,"ontCAGE.Neuron_THP1.CRE.stranded_summit_50bp_extend.INR.fakebed.tsv.gz")), row.names=F, col.names=T, sep="\t", quote=F)
INR$locE=INR$locS+2
...
for (i in 1:length(need)){
  ...
  INRp1D1=INRp1D1%>%group_by(CREID)%>%dplyr::slice_max(motif_score)
  INRp2D1=INRp2D1%>%group_by(CREID)%>%dplyr::slice_max(motif_score)
  INRe1=INRe1%>%group_by(CREID)%>%dplyr::slice_max(motif_score)
  INRe1D1=INRe1D1%>%group_by(CREID)%>%dplyr::slice_max(motif_score)
  INRe2D1=INRe2D1%>%group_by(CREID)%>%dplyr::slice_max(motif_score)
  INRu1=INRu1%>%group_by(CREID)%>%dplyr::slice_max(motif_score)
  path2=paste0(INR_path,"n",i,"/")
  write.table(INRc1[order(INRc1$locS),c(1:6)],gzfile(paste0(path2,"summit_50bp_extend.INRc.nooverlap.fakebed.bed.gz")), row.names=F, col.names=F, sep="\t", quote=F)
  write.table(INRp1[order(INRp1$locS),c(1:6)],gzfile(paste0(path2,"summit_50bp_extend.INRp.nooverlap.fakebed.bed.gz")), row.names=F, col.names=F, sep="\t", quote=F)
  write.table(INRe1[order(INRe1$locS),c(1:6)],gzfile(paste0(path2,"summit_50bp_extend.INRe.nooverlap.fakebed.bed.gz")), row.names=F, col.names=F, sep="\t", quote=F)
  write.table(INRu1[order(INRu1$locS),c(1:6)],gzfile(paste0(path2,"summit_50bp_extend.INRu.nooverlap.fakebed.bed.gz")), row.names=F, col.names=F, sep="\t", quote=F)
  write.table(INRp2D1[order(INRp2D1$locS),c(1:6)],gzfile(paste0(path2,"summit_50bp_extend.INRp2D.nooverlap.fakebed.bed.gz")), row.names=F, col.names=F, sep="\t", quote=F)
  write.table(INRp1D1[order(INRp1D1$locS),c(1:6)],gzfile(paste0(path2,"summit_50bp_extend.INRp1D.nooverlap.fakebed.bed.gz")), row.names=F, col.names=F, sep="\t", quote=F)
  write.table(INRe2D1[order(INRe2D1$locS),c(1:6)],gzfile(paste0(path2,"summit_50bp_extend.INRe2D.nooverlap.fakebed.bed.gz")), row.names=F, col.names=F, sep="\t", quote=F)
  write.table(INRe1D1[order(INRe1D1$locS),c(1:6)],gzfile(paste0(path2,"summit_50bp_extend.INRe1D.nooverlap.fakebed.bed.gz")), row.names=F, col.names=F, sep="\t", quote=F)}

#================================
#bedtools count
#bash
setwd(paste0(INR_path,"n1/"))
system(paste0("for file in *nooverlap.fakebed.bed.gz; do bedtools intersect -wa -a ",INR_path,"INR.location101.fakebed.bed.gz -b \"$file\" -c | gzip > \"${file%.fakebed.bed.gz}.result.bed.gz\"; done"))

setwd(paste0(INR_path,"n2/"))
system(paste0("for file in *nooverlap.fakebed.bed.gz; do bedtools intersect -wa -a ",INR_path,"INR.location101.fakebed.bed.gz -b \"$file\" -c | gzip > \"${file%.fakebed.bed.gz}.result.bed.gz\"; done"))

setwd(paste0(INR_path,"n3/"))
system(paste0("for file in *nooverlap.fakebed.bed.gz; do bedtools intersect -wa -a ",INR_path,"INR.location101.fakebed.bed.gz -b \"$file\" -c | gzip > \"${file%.fakebed.bed.gz}.result.bed.gz\"; done"))

setwd(paste0(INR_path,"n4/"))
system(paste0("for file in *nooverlap.fakebed.bed.gz; do bedtools intersect -wa -a ",INR_path,"INR.location101.fakebed.bed.gz -b \"$file\" -c | gzip > \"${file%.fakebed.bed.gz}.result.bed.gz\"; done"))
...
size=rep(size,4)
files=list.files(path=INR_path, pattern="nooverlap.result.bed.gz", recursive=T)
files.index=sapply(strsplit(files, "\\/"),"[",1)
files.names=sapply(strsplit(files, "extend."),"[",2)
files.names=gsub(".result.bed.gz","",files.names)
data=read.delim(paste0(INR_path, files[1]), header=F, stringsAsFactors = F)
data$group=files.names[1]
data$signalID=files.index[1]
data$V5=data$V5/size[1]
for (i in 2:length(files)){
  data1=read.delim(paste0(INR_path, files[i]), header=F, stringsAsFactors = F)
...
data$group="INR"
write.table(data, gzfile(paste0(path_fig2_data,"INR_nooverlap_result.tsv.gz")), col.names=T, row.names=F, sep="\t", quote=F)
