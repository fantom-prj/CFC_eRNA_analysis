...
my $baseDir = '/analysisdata/fantom6/Interactome/ONT.CAGE.satellite/dorado_run/SCAFE/20231126/ont_ss_Neuronalone/aggregate/run_full';
my $run_script = '/analysisdata/fantom6/Interactome/ONT.CAGE.satellite/dorado_run/SCAFE/20231126/ont_ss_Neuronalone/script/00_run_full.sh';
my $ctss_list_path = '/analysisdata/fantom6/Interactome/ONT.CAGE.satellite/dorado_run/SCAFE/20231126/ont_ss_Neuronalone/aggregate/pool_runs/ctss_bed/00_pool_runs.ctss_list.tsv';
...
system "mkdir -pm 777 $baseDir";
system "mkdir -pm 777 $sbatch_dir";
system "mkdir -pm 777 $aggregate_list_dir";
system "cp $0 $baseDir";
...
while (<INCTSSLIST>) {
    ...
    my $all_scope = "ont_ss.Neuronalone";
    $scope_info_hsh_ref->{$all_scope}{$libID} = [$all_ctss_path, $ung_ctss_path];
}
close INCTSSLIST;
...
foreach my $scopeID (sort keys %{$scope_info_hsh_ref}) {
    ...
    foreach my $libID (sort keys %{$scope_info_hsh_ref->{$scopeID}}) {
        ...
        print AGRLIST join "\t", ($libID, $all_ctss_path, $ung_ctss_path), "\n";
    }
    close AGRLIST;
