...
my $outDir = "/analysisdata/fantom6/Interactome/ONT.CAGE.satellite/dorado_run/exosome_sensitivity/kallisto_20240904/kallisto_quantification";
my $in_fq_list_path = "/analysisdata/fantom6/Interactome/ONT.CAGE.satellite/dorado_run/exosome_sensitivity/interactome_RNAseq_fastq_path.txt";
...
open FQLIST, "<", $in_fq_list_path;
while (<FQLIST>) {
	chomp;
	next if $_ =~ m/^#/;
	my ($libID, $fq_path1, $fq_path2) = split /\t/;
	$lib_info_hsh_ref->{$libID} = [$fq_path1, $fq_path2];
...
foreach my $indexID (keys %{$kallisto_index_hsh_ref}) {
	...
	foreach my $libID (sort keys %{$lib_info_hsh_ref}) {
		my $fq_path1 = $lib_info_hsh_ref->{$libID}[0];
		my $fq_path2 = $lib_info_hsh_ref->{$libID}[1];

		
		die "$fq_path1 of $libID is not defined\n" if not -s $fq_path1;
